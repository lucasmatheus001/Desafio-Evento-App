...
export default function EditEvent() {
  ...
  useEffect(() => {
    api.get(`/events/${uuid}`).then((res) => {
      const event = res.data;
      setName(event.name);
      setDescription(event.description);
      setZipCode(event.zipcode);
      setNumber(event.number);
      setStartsAt(event.starts_at?.substring(0, 10));
      setEndsAt(event.ends_at?.substring(0, 10));
      setMaxSubscription(event.max_subscription);
